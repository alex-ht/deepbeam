...
def train():
    ...
    with tf.Graph().as_default(), tf.device('/device:CPU:0'):
        ...
        data_segments = tfio.IODataset.from_hdf5(
            filename='assets/noisy_train.mat', dataset='noisy_train')
        data_labels = tfio.IODataset.from_hdf5(
            filename='assets/target_train.mat', dataset='target_train')
            ...
            for i in xrange(NUM_GPUS):
                with tf.device('/device:GPU:%d' % i):
                    with tf.name_scope('%s_%d' %
                        ...
                        # all towers.
                        loss = tower_loss(scope, segments.numpy(),
                                          labels.numpy())
        ...
        summary_op = tf.summary.merge(summaries,
                                      collections=[tf.GraphKeys.SUMMARY_OP])
        ...
        with sv.managed_session(config=sess_config,

...
def train():
    ...
    with tf.device('/cpu:0'):
        ...
        input_segments = tfio.IODataset.from_hdf5('assets/noisy_train.mat', '/noisy_train')
        input_labels = tfio.IODataset.from_hdf5('assets/target_train.mat', '/target_train')
        input_ds = tf.data.Dataset.zip((input_segments, input_labels)).repeat()
        ...
        lr = tf.keras.optimizers.schedules.ExponentialDecay(INITIAL_LEARNING_RATE,
                                        NUM_STEPS_PER_DECAY, ANNEALING_RATE, staircase=True)

        # Create an optimizer that performs gradient descent.
        opt = tf.keras.optimizers.Adam(lr)

        segments = tf.keras.Input(shape=(24570,))
        with tf.device('/cpu:0'):
            # Get batches of images and labels for BAWN.
            #segments, labels = input_ds.batch(bawn.BATCH_SIZE)
            ...
            # all towers.
            loss = tower_loss(, segments, labels)

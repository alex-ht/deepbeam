...
def _embed_conv1d(inputs, out_channels, activation=tf.tanh, bias=True):
    ...
    with tf.variable_scope('pre') as scope:
        ...
        embedded_kernel = tf.nn.embedding_lookup(ww,
                                                 inputs,
                                                 name='embedded_kernel')

        output = tf.add(embedded_kernel[:, 0:-1, :, 0],
                        embedded_kernel[:, 1:, :, 1],
                        name='shift_and_add')
        ...
        if bias:
            ...
            b = _variable_on_cpu('bias', [len_in - 1],
...
def _dilated_conv1d(inputs,
    ...
    with tf.variable_scope(name):
        ...
        outputs_filter = tf.slice(outputs_together, [0, 0, 0],
                                  [-1, residual_channels, -1],
                                  name='filter_part')
        outputs_gate = tf.slice(outputs_together, [0, residual_channels, 0],
                                [-1, -1, -1],
                                name='gate_part')
        ...
        outputs_gated = tf.multiply(outputs_filter,
                                    outputs_gate,
                                    name='filter_X_gate')
        ...
        if not top:
            ...
            outputs_dense = tf.add(inputs_proc[:, :, ind_in],
                                   outputs_residual,
                                   name='add_residual')
        else:
            outputs_dense = None
...
def _post_processing(inputs,
    ...
    with tf.variable_scope('{}post_l{}'.format(name, num_layers - 1),
                           reuse=reuse):
        outputs = _conv1d(h,
                          out_channels=num_classes,
                          filter_width=1,
...
def _post_processing_batch(inputs,
                           num_layers,
                           num_classes,
                           name,
                           reuse=False,
    ...
    inputs_agg = tf.reshape(inputs, [batch_size, -1, width],
                            name='skips_batch')
...
def load_data_prior(train, target):
    ...
    assert (inputs.dtype == 'uint8'
            and labels.dtype == 'uint8'), 'Data type incorrect!!!!'
    assert inputs.shape[0] == labels.shape[
        0], 'The first dimension (batch size) must equal!!!!'
...
def data_initializer_prior(data_segments, data_labels):
    # Input data
    segments_initializer = tf.placeholder(tf.uint8,
    ...
    input_segments = tf.Variable(segments_initializer,
                                 trainable=False,
    ...
    return (segments_initializer, labels_initializer, input_segments,
            input_labels)
...
def inputs_batch_prior(input_segments, input_labels):
    ...
    segment, label = tf.train.slice_input_producer(
...
def load_data_likli(clean, noisy, target):
    ...
    assert (inputs_clean.dtype == 'uint8' and inputs_noisy.dtype == 'float32'
            and labels.dtype == 'uint8'), 'Data type incorrect!!!!'
    assert inputs_clean.shape[0] == inputs_noisy.shape[0] == labels.shape[
        0], 'The first dimension (batch size) must equal!!!!'
...
def data_initializer_likli(data_clean, data_noisy, data_labels):
    # Input data
    clean_initializer = tf.placeholder_with_default(tf.zeros(
        data_clean.shape, tf.uint8),
                                                    shape=data_clean.shape,
                                                    name='clean_initializer')
    noisy_initializer = tf.placeholder_with_default(tf.zeros(
        data_noisy.shape, tf.float32),
                                                    shape=data_noisy.shape,
                                                    name='noisy_initializer')
    labels_initializer = tf.placeholder_with_default(tf.zeros(
        data_labels.shape, tf.uint8),
                                                     shape=data_labels.shape,
                                                     name='labels_initializer')
    # variables
    input_clean = tf.Variable(clean_initializer,
                              trainable=False,
                              collections=[tf.GraphKeys.LOCAL_VARIABLES],
                              name='input_clean')
    input_noisy = tf.Variable(noisy_initializer,
                              trainable=False,
                              collections=[tf.GraphKeys.LOCAL_VARIABLES],
                              name='input_noisy')
    input_labels = tf.Variable(labels_initializer,
    ...
    return (clean_initializer, noisy_initializer, labels_initializer,
            input_clean, input_noisy, input_labels)
...
def inputs_batch_likli(input_clean, input_noisy, input_labels):
    ...
    clean, noisy, label = tf.train.slice_input_producer(
        [input_clean, input_noisy, input_labels])
    cleans, noisys, labels = tf.train.batch([clean, noisy, label],
...
def model_denoise(inputs_clean, inputs_noisy):
    ...
    outputs_ll_batch = _post_processing(skips_ll_batch, NUM_POST_LAYERS,
                                        NUM_CLASSES, 'likli/')
    ...
    outputs_loglik_batch = tf.stop_gradient(
        outputs_pr_batch) + outputs_ll_batch
...
def model_simple(inputs_noisy):
    ...
    outputs_sp_batch = _post_processing(skips_noisy_batch, NUM_POST_LAYERS,
                                        NUM_CLASSES, 'noisy/')
...
def _step_filter(inputs,
    ...
    if width == 2:
        ...
        output = tf.matmul(inputs, w_e) + tf.matmul(state, w_r) + tf.matmul(
            future, w_f)
    else:
        w = w[0, :, :]
...
def dilated_generation(inputs,
    ...
    with tf.variable_scope(name, reuse=True) as scope:
        ...
        output_together = _step_filter(inputs,
                                       state,
                                       future,
                                       width,
                                       bias=False,
        ...
        #slice 2r channels into two r channels
        output_filter = tf.slice(output_together, [0, 0],
                                 [-1, residual_channels],
                                 name='filter_part')
        output_gate = tf.slice(output_together, [0, residual_channels],
                               [-1, -1],
                               name='gate_part')
        #add activations
        output_filter = tf.tanh(output_filter, name='filter')
        output_gate = tf.sigmoid(output_gate, name='gate')
        output_gated = tf.multiply(output_filter,
                                   output_gate,
                                   name='filter_X_gate')

        #output_dense = residual + input
        if not top:
            output_residual = _step_filter(output_gated,
                                           None,
                                           None,
                                           1,
                                           name='residual')
...
def _causal_generate(self,
    ...
    for b in xrange(num_blocks):
        for i in xrange(num_layers):
            ...
            h, skip = bawn.dilated_generation(h,
                                              state_,
                                              None,
                                              width=2,
                                              top=top,
...
def step_generation(inputs_clean, skips_noisy, batch_size):
    ...
    outputs_pr = post_processing_generation(skips_prior, NUM_POST_LAYERS,
                                            'prior/')
    outputs_ll = post_processing_generation(skips_likli, NUM_POST_LAYERS,
                                            'likli/')
...
def run_semi_online_v2(sess, out_ops, skips_noisy_batch, indices, inputs_noisy,
                       num_samples):
    skips_noisy_sum = sess.run(skips_noisy_batch)
    predictions_ = []
    for step in xrange(num_samples):
        feed_dict = feed_dict = {
            self.inputs_clean: indices,
